...
{{- define "get_sso_config" -}}
  {{- $secret := (lookup "v1" "Secret" .context.Values.ssoProviderNamespace .context.Values.configSecretName) }}
  {{- if $secret }}
    ...
    {{- end -}}
  {{- else }}
    {{- printf "\nERROR: secret \"%s\" does not exist in namespace \"%s\"\n" .context.Values.configSecretName .context.Values.ssoProviderNamespace | fail -}}
  {{- end -}}
{{- end -}}
